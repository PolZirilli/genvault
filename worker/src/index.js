...
async function fetchGamesJson(cfg, token) {
    ...
    return { games, sha: current.sha, contentsUrl };
}

// Busca un juego con el mismo nombre (normalizado) que no sea el mismo id —
// eso es lo único que cuenta como "duplicado": mismo id = edición/sobreescritura
// intencional, y cada plataforma tiene su propio games.json así que un mismo
// nombre en consolas distintas nunca colisiona acá.
function findDuplicateName(games, name, excludeId) {
    const target = String(name).trim().toLowerCase();
    return games.find(g => g.id !== excludeId && String(g.name || '').trim().toLowerCase() === target);
}

async function commitGamesJson(cfg, games, sha, contentsUrl, entry, token) {
...
async function handleUpload(request, env, corsHdrs) {
    ...
    const log = [];
    try {
        // 0) Traer data/games.json ANTES de tocar nada — sirve para chequear
        // duplicados por nombre dentro de esta misma plataforma/consola y, si todo
        // sale bien, para el commit final (evita un segundo fetch más adelante).
        let gamesState;
        try {
            gamesState = await fetchGamesJson(cfg, env.GITHUB_TOKEN);
        } catch (e) {
            return jsonResponse({ error: `No pude leer data/games.json de ${cfg.repo}: ${e.message}` }, 500, corsHdrs);
        }
        const dup = findDuplicateName(gamesState.games, name, id);
        if (dup) {
            return jsonResponse(
                {
                    error: `Ya existe "${dup.name}" en ${cfg.label} (id "${dup.id}"). Si es el mismo juego, subilo con ese mismo id para actualizarlo; si es otro juego, cambiale el nombre.`,
                },
                409,
                corsHdrs
            );
        ...
        // 3) Commitear data/games.json en GitHub (reutiliza lo que ya trajimos en el paso 0)
        const entry = { id, name, region, cover, url: romUrl };
        const result = await commitGamesJson(cfg, gamesState.games, gamesState.sha, gamesState.contentsUrl, entry, env.GITHUB_TOKEN);
        log.push({
            kind: 'ok',
            msg:
